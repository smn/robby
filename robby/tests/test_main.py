...
class TestRobby(TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(self.robby.bayes.flush, bucket='bucket')
    ...
    def test_classify(self):
        yield self.robby.bayes.train('sample1', SAMPLE_TEXT_1, bucket='bucket')
        yield self.robby.bayes.train('sample2', SAMPLE_TEXT_2, bucket='bucket')
        response = yield self.request(
            'POST', '/classify/bucket', data=SAMPLE_TEXT_1)
    ...
    @inlineCallbacks
    def test_train(self):
        yield self.request('POST', '/train/bucket/sample1', data=SAMPLE_TEXT_1)
        yield self.request('POST', '/train/bucket/sample2', data=SAMPLE_TEXT_2)
        response = yield self.request(
            'POST', '/classify/bucket', data=SAMPLE_TEXT_1)
        data = yield response.json()
        self.assertEqual(data, {
    ...
    def test_batch_train(self):
        yield self.request('POST', '/batch/train/bucket', data=json.dumps([
    ...
    def test_untrain(self):
        yield self.request('POST', '/train/bucket/sample1', data=SAMPLE_TEXT_1)
        yield self.request(
            'POST', '/untrain/bucket/sample1', data=SAMPLE_TEXT_1)
        response = yield self.request(
            'POST', '/classify/bucket', data=SAMPLE_TEXT_1)
        data = yield response.json()
        self.assertEqual(data, {
    ...
    def test_score(self):
        yield self.request('POST', '/train/bucket/sample1', data=SAMPLE_TEXT_1)
        response = yield self.request(
            'POST', '/score/bucket', data=SAMPLE_TEXT_1)
        data = yield response.json()
    ...
    def test_flush(self):
        ...
        self.assertTrue(
            (yield self.redis.sismember(
                self.robby.bayes.key('bucket', 'categories'), 'sample1')))
        response = yield self.request('DELETE', '/flush/bucket')
        data = yield response.json()
